...
const boolean_map = {"test-keys": false, "unknown": false};
//Helper function
...
const ClassUse = Java.use("java.lang.Class");
...
const ProcessImpl = Java.use("java.lang.ProcessManager$ProcessImpl");
...
//Anti-emulator implementation
Build.TAGS.value = 'release-keys';
Build.BOARD.value = 'hammerhead';
Build.BRAND.value = 'hammerhead';
Build.DEVICE.value = 'hammerhead';
Build.FINGERPRINT.value = 'google/hammerhead/hammerhead:4.4.4/KTU84P/1227136:user/release-keys';
Build.HARDWARE.value = 'HKUFYPhone';
Build.ID.value = 'KTU84P';
Build.MANUFACTURER.value = 'LGE';
Build.MODEL.value = 'HKUPhone';
Build.PRODUCT.value = 'hammerhead';
Build.SERIAL.value = '23124214';
Build.USER.value = 'user';
...
ActivityManager.getRunningServices.implementation = function (max_int) {
    var r_list = [];
    var List = Java.use('java.util.List');
    var RunningServiceInfo = Java.use('android.app.ActivityManager.RunningServiceInfo');
    var list = this.getRunningServices.call(this, max_int);
    var list = Java.cast(list, List);
    for(var i = 0; i < list.size(); i++){
        var currentElement = Java.cast(list.get(i), RunningServiceInfo);
        if(currentElement.process.value in RootBinaries){
            r_list.push(i);
        }
    }
    ...
    if(r_list.length > 0) {
        ...
        droidmon_log("Bypass getRunningServices", {
            "class": "android.app.ActivityManager",
            ...
            "args": "",
            "result": "",
            "hook_result": "Remove index: " + r_list.toString()
        });
    }
    return list;
};

PackageManager.getPackageInfo.implementation = function (packagename, id) {
    if(knownRootCloakingPackages.indexOf(packagename) > -1){
        var date = new Date(); var timestamp = date.getTime();
        droidmon_log("Bypass getPackageInfo",{
                "class":"android.app.ApplicationPackageManager",
                "method":"getPackageInfo",
                "timestamp": timestamp,
                "type":"content",
                "args": [packagename, id],
                "result": this.getPackageInfo.call(this, packagename, id),
                "hook_result": "throw NameNotFoundException"
            });
...
Location.getLatitude.implementation = function() {
    var date = new Date(); var timestamp = date.getTime();
    droidmon_log("Bypass getLatitude",{
        "class":"android.location.Location",
        "method":"getLatitude",
        "timestamp": timestamp,
        "type":"content",
        "args":"",
        "result": this.getLatitude.call(this),
        "hook_result": lat
    });
    return lat;
}
Location.getLongitude.implementation = function() {
    var date = new Date(); var timestamp = date.getTime();
    droidmon_log("Bypass getLongitude",{
        "class":"android.location.Location",
        "method":"getLongitude",
        "timestamp": timestamp,
        "type":"content",
        "args":"",
        "result": this.getLongitude.call(this),
        "hook_result": lng
    });
    return lng;
}
...
TelephonyManager.getNetworkOperator.overload().implementation = function () {
    var date = new Date(); var timestamp = date.getTime();
     droidmon_log("Bypass getNetworkOperator",{
                "class":"android.telephony.TelephonyManager",
...
TelephonyManager.getNetworkCountryIso.overload().implementation = function () {
    var date = new Date(); var timestamp = date.getTime();
     droidmon_log("Bypass getNetworkCountryIso",{
                "class":"android.telephony.TelephonyManager",
...
TelephonyManager.getSubscriberId.overload().implementation = function () {
    var date = new Date(); var timestamp = date.getTime();
     droidmon_log("Bypass getSubscriberId",{
                "class":"android.telephony.TelephonyManager",
                ...
                "args": "",
                "result": "",
                "hook_result": "525040773654732"
            });
    return "525040773654732";
};
TelephonyManager.getNetworkType.overload().implementation = function () {
    var date = new Date(); var timestamp = date.getTime();
     droidmon_log("Bypass getNetworkType",{
                "class":"android.telephony.TelephonyManager",
                "method":"getNetworkType",
                "timestamp": timestamp,
                "type":"content",
                "args": "",
                "result": this.getNetworkType.call(this),
                "hook_result": 13
            });
    return 13;
};
TelephonyManager.getPhoneType.overload().implementation = function () {
     var date = new Date(); var timestamp = date.getTime();
     droidmon_log("Bypass getPhoneType",{
                "class":"android.telephony.TelephonyManager",
                "method":"getPhoneType",
                "timestamp": timestamp,
                "type":"content",
                "args": "",
                "result": this.getPhoneType.call(this),
                "hook_result": 2
            });
    return 2;
};
TelephonyManager.getVoiceMailNumber.overload().implementation = function () {
     var date = new Date(); var timestamp = date.getTime();
     droidmon_log("Bypass getVoiceMailNumber",{
                "class":"android.telephony.TelephonyManager",
...
FileIO.exists.implementation = function() {
    var name = FileIO.getName.call(this);
...
ProcessImpl.getInputStream.implementation = function () {
    ...
    if (uid > 10000 && uid < 99999) {
        if (this.getInputStream.call(this) === Java.cast(FileInputStream.$new("/system/build.prop"), InputStream)) {
            var date = new Date(); var timestamp = date.getTime();
...
/*Debugging for Android 6.0
const Context = Java.use("android.content.Context");
Context.getFilesDir.implementation = function () {
    send_msg("Called getFileDir");
    this.getFilesDir.call(this);
    return this.getFilesDir.call(this);
};
const htHttpURLConnectionImplml = Java.use("com.android.okhttp.internal.huc.HttpURLConnectionImpl");
const URLConnection = Java.use("java.net.URLConnection");
htHttpURLConnectionImplml.setRequestProperty.implementation = function (field, newValue) {
    send_msg("Called setRequestProperty"+field+" "+newValue);
    //this.disconnect.call(this);
    return this.setRequestProperty.call(this,field,newValue);
};
*/
    ...
    onEnter: function(args) {
        ...
        if (shouldFakeReturn) {
            ...
            droidmon_log("Bypass fopen",{
                ...
                "timestamp": timestamp,
                "type":"content",
                "args": Memory.readCString(args[0]),
                "result": path,
